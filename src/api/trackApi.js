...
export const getTrackPlayback = async (trackId) => {
  const res = await apiClient.get(`/api/tracks/${trackId}/playbacks`);
  return res.data;
};

//3. 재생 카운트 기록 POST(옵션)
export const postTrackPlayback = async (trackId) => {
  const res = await apiClient.post(`/api/tracks/${trackId}/playbacks`);
  return res.data;
};
...
export const getTrackLyrics = async (trackId) => {
  const res = await apiClient.get(`/api/tracks/${trackId}/lyrics`);
  return res.data;
};
